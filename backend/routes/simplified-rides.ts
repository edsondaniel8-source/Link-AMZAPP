  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Campos obrigatórios: origem, destino, data, preço e lugares disponíveis",
      });
    }
...
router.post("/book", async (req, res) => {
  try {
    const { rideId, passengerId, seatsBooked, phone, email, notes } = req.body;

    // Validações
    if (!rideId || !passengerId || !seatsBooked) {
      return res.status(400).json({
        error:
          "Campos obrigatórios: ID da viagem, passageiro e número de lugares",
      });
    }
    ...
    if (ride.available_seats < seatsBooked) {
      return res
        .status(400)
        .json({ error: "Não há lugares suficientes disponíveis" });
    }
