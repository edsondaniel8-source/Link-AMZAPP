...
export class DatabaseStorage {
    ...
    async searchAccommodations(location, checkIn, checkOut) {
    ...
    async updateBookingStatus(id, status, details) {
        const [booking] = await db
            .update(bookings)
    ...
    async approveBooking(bookingId, providerId) {
        const [booking] = await db
            .update(bookings)
            .set({
            status: "approved",
...
export class MemStorage {
    ...
    async getProviderBookings(providerId) {
        const providerBookings = Array.from(this.bookings.values()).filter(booking => booking.providerId === providerId);
