...
export async function registerRoutes(app) {
    ...
    app.post('/api/auth/complete-registration', upload.fields([
        ...
        try {
            const { firstName, lastName, phone, email, documentType, documentNumber } = req.body;
    ...
    app.get("/api/rides/search", async (req, res) => {
        try {
            ...
            const rides = await storage.searchRides(from, to);
            res.json(rides);
        }
        catch (error) {
    ...
    app.get("/api/accommodations/search", async (req, res) => {
        try {
            ...
            const accommodations = await storage.searchAccommodations(location, checkInDate, checkOutDate);
