...
export function createApiServer() {
  ...
  app.post('/api/rides', (req, res) => {
    console.log('📝 POST /api/rides - Criando nova rota');
    ...
    try {
      ...
      const newRoute = {
        ...
        departureDate: date && time ? `${date}T${time}:00.000Z` : new Date().toISOString(),
        createdAt: new Date().toISOString(),
        status: 'published',
        ...req.body
      };
  ...
  app.get('/api/rides', (req, res) => {
    console.log('📋 GET /api/rides - Listar todas as rotas');
    res.json({
      rides: routes,
      total: routes.length,
      timestamp: new Date().toISOString()
